var pub = {},
    ...
    cacheGenerators = [],
    _, Q, config, template, promises, options, requireHelper;
...
function create(domainConfig) {
    var result = [],
        generatorConfig, generator, generatorPromise;
    ...
    if (!cacheGenerators[getGeneratorName(domainConfig.generator)]) {
        cacheGenerators[getGeneratorName(domainConfig.generator)] = generator.createData();
    }
    generatorPromise = cacheGenerators[getGeneratorName(domainConfig.generator)];
...
function getTemplateFunction(templateFile, delimiter, element) {
    return function() {
        try {
            template.loadCustomScript(config.get('customScript'));
        } catch (error) {}
...
function getGenerator(generator) {
    var generatorName = getGeneratorName(generator),
        Generator = requireHelper.require(generatorName, GENERATOR_PREFIX);
